...
1. Apex calls `POST /v1/csv/init` with a ContentVersion ID and the `CursorBatch_Job__c` record ID
...
```json
{
  "Job_Record_Id__c": "a1fPV000008hHgxYAE",
  "Coordinator_Class__c": "CSV_Ready",
  "Job_Name__c": "cq_abc123..."
}
```

- **`Job_Record_Id__c`** — the `CursorBatch_Job__c` record ID passed in the init request (`cursorBatchJobId`), so the framework can correlate the callback to the correct job.
- **`Coordinator_Class__c`** — hardcoded to `CSV_Ready`. The Apex coordinator class that handles the callback.
- **`Job_Name__c`** — the `csvQueryId` generated by the middleware, used to query the status endpoint for full session details.
...
**Salesforce prerequisite:** The org must have the `CursorBatch_Coordinator__e` Platform Event (included in the cursor-batch-framework managed package) with fields `Job_Record_Id__c` (Text), `Coordinator_Class__c` (Text), and `Job_Name__c` (Text).
...
| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/v1/csv/init` | Start a Cursor CSV session. Body: `{ contentVersionId, cursorBatchJobId }`. Returns 202 with `csvQueryId`. The middleware automatically publishes a Platform Event when the session is ready or errors. |
